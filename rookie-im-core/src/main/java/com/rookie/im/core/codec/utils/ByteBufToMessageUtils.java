...
public class ByteBufToMessageUtils {
    ...
    public static Message transition(ByteBuf in) {
        ...
        MessageHeader messageHeader = createMessageHeader(command, version, clientType, messageType, appId,
                imeiLength, imei, bodyLen);
        Message message = createMessage(messageHeader, messageType, bodyData);

        return message;
    }

    private static Message createMessage(MessageHeader messageHeader, int messageType, byte[] bodyData) {
